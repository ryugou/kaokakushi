...
**`resolveOrphanedAttempts` が解決後の全件を返します。** 解決した attempt だけを返すと、呼び出し側が「解決結果を復旧前の snapshot へ手で反映する」ことになり、反映漏れが型で検出できません。**戻り値がその時点の受け渡し状態の正であり、起動時復旧の手順 7.5 と 8 はこれを入力にします**（5 章）。
...
**読み取りは 3 回に分かれます。** それぞれ入力を必要とする手順が違い、その間に DB が変わるためです。
...
**手順 2 の読み取りを `RecoverySnapshot` で兼ねません。** 手順 2.5（台帳修復時の破棄。5.2）は非終端 `ExportCommit` を全削除し、全 `WorkingSourceRecord` を削除し、キュー項目を `failed(ledgerRepaired)` へ遷移させます。**手順 2 の時点で `RecoverySnapshot` を読むと、手順 4 が「2.5 が破棄したはずのコミット」を反復し、修復起動で捨てると決めた作業を復活させます。**

**手順 2 が必要とするのはコミット行だけです。** 署名検証と「どれが非終端か」の判定に `OutputRecord` や `WorkingSourceRecord` は要りません。**軽い専用ポートを分けることで、重い snapshot を捨てて読み直す無駄も消えます。**

**`RecoverySnapshot` はコミット復旧（手順 3・4）の入力です。** 手順 2.8 で一度だけ読み、その間に個別クエリを繰り返しません。`hasSignatureInvalidCommit`（手順 3 の保留判定に使う）は `RecoverySnapshot` にも持たせます（下記）。
...
**手順 4.5 の無効化が起こす変化は、手順 5.5 の内側で読み直します。** 4.5 は照合失敗時に `WorkingSourceRecord` を破棄しますが、その台帳側（binding 削除）だけが失敗する可能性があります。**手順 5.5 は「`WorkingSourceRecord` が無い孤児 binding」を、4.2 の snapshot ではなくその時点の DB から判定します。** 孤児回収は自分が消す対象を自分で読む、が原則です。
...
**`loadPostCommitRecoverySnapshot()` を手順 4 の完了直後に 1 回だけ呼びます。** 手順 4.5〜7 は毎回のクエリではなくこの値を使います。

**手順 7 も DB を変えます**（`OutputRecord.state` の更新、`DeliveryAttempt` の削除、`UnknownLibrarySave` の追加）。したがって、**手順 7.5 と 8 はこの snapshot ではなく、`resolveOrphanedAttempts()` の戻り値（解決後の全 `OutputDeliverySnapshot`）を使います**（0 章）。snapshot は 2 回、受け渡し状態は手順 7 の戻り値、で全手順の入力が最新になります。
...
`FinalizeExportInput` は `OutputRecord` も `ExportRecord` も受け取りません。**受け取ったうえで使わない契約は、呼び出し側に「渡した値が書かれる」と誤解させます。** 渡すのは、コミットから導出できない 2 つだけです。
...
**`projectUpdatedAt` も渡しません。** `commit.finalizedAt` から導出できます。別の時刻を渡せると、`Project` の更新時刻だけを任意に操作できます。
...
**`OutputRecord.projectID` の UNIQUE 制約を事前検査します**（[アーキテクチャ設計](architecture.md) の 7.1）。未削除の `delivered` 出力が残る `Project` を再書き出しすると、手順 4（台帳への暫定適用）まで進んだ後に手順 7 が制約違反で失敗します。**他のすべての事前条件が「手順 7 を実行しない」ための明示検査として列挙されているのに、この 1 件だけ実行時例外として現れるのを避けます。** 検査で弾けば `AppErrorCode` も `databaseFailure` ではなく実態に合う分類になります。

**通常は先に解消されます。** 未受け渡し出力がある状態では新しい加工を開始できず（同 7.5）、`delivered` は完了画面を離れた時点で削除されます。この検査は、その規則が破れた場合の最後の防御です。

**キュー項目も同じトランザクション内で検査します。** `queueItemID` は呼び出し側が指定する値であり、検査しなければ**無関係な `Project` のキュー項目を `completed` へ更新できます。**
...
**順序は台帳を触らない検査が先です。** 前 3 つで弾けば、補償が不要になります。

### 1.1.2 確認の成立を認可で再導出する

**開始条件が保存された `reviewed` を根拠にできません。** `DetectionStatus` と `ReviewStatus` は未署名の `app.db` 行であり、書き換えれば確認を経ずに通ります（[アーキテクチャ設計](architecture.md) の 6.1）。
...
| バッチ（1 枚ずつ確認） | 上記に加えて**全写真**について上記が成立している |

**保存された `reviewed` は使いません。** 1.1 の開始条件表にある「モードごとの確認条件」は、この再導出の結果として評価します。**`reviewed` は表示の高速化のためのキャッシュです。**

**`triage` は純粋関数であり、再実行は顔数に比例するだけです**（Vision の呼び出しを伴いません）。50 枚のバッチでも無視できる費用です。

**再導出の入力自体（`FaceTrack`）も未署名です。** その限界と、それでも再導出する理由は [アーキテクチャ設計](architecture.md) の 6.1 にあります。

### 1.1.1 設定内容の能力を認可で再検査する

**編集画面の `canEdit` だけでは、有料スタンプの利用を防げません。** `EffectSetting` は未署名の DB 行であり、`op` を `.stamp(source: .builtIn(code: "<有料パックの code>"), opacity: 1.0)` へ書き換えて `projectRevision` を手で増やせば、UI を通らずに有料スタンプを含む `RenderSpec` ができます。プレビューも通常どおり表示され、**書き出しは `freeMonthlyConsume` として成立します。**
...
**`capabilityRequirement(of:)` が `nil` を返しません。** カタログに無い `code` は `.unknownBuiltIn` として `Set` へ入れます。`Optional` を返すと、呼び出し側が `nil` を「制約なし」と解釈する経路ができます。**`ProjectCapabilityRequirement` は「必要能力の集合」であり、判定できなかったことも 1 つの要素として表します。**
...
**手順 −2 の 1 回だけでは、`prepared` で停止させて設定を差し替えられます。** `paused(.userPaused)` は手順 1 の前の保存点であり時間制限がありません。その間に `EffectSetting.op` を有料スタンプへ書き換え、**`Project.projectRevision` を据え置けば** `expectedProjectRevision`（手順 0 でしか照合しない）も `ExportCommit`（`RenderSpec` を持たない）も検出できません。
...
**「認可した spec」と「描画した spec」を同一にするのがこの検査の目的です。** 手順 1 は DB から `RenderSpec` を組み立てるため、その組み立て結果を検査します。**検査（2）を加えたことで、能力要件に影響しない設定変更も検出されます。** `ExportCommit` へ `RenderSpec` を持たせて署名する方法もありますが、`RenderSpec` は領域数に比例して大きく、コミット行の再署名コストが増えます。**描画の直前に純粋関数を 2 回呼ぶほうが安価です。**

**検査（1）だけでは不十分です。** `ProjectCapabilityRequirement` はスタンプの必要能力の集合でしかなく、**領域の縮小、`cellRatio` の最小化、`isMasked` の解除は素通りします。** それらは[アーキテクチャ設計](architecture.md) の 6.1 で「自分の写真に対する自己判断の省略」として脅威モデルの対象外に整理されていますが、**同じ改変が `settingsEntryToApply` を経由して台帳の確定記録を汚染する経路は対象外ではありません。** 検査（2）はそちらを塞ぎます。
...
**`unknownBuiltInStampCode` に `UpgradeReason` を作りません。** カタログに無い `code`（DB 改変またはダウングレード後の未知データ）であり、**課金しても `authorized` になりません。** Paywall を提示すると、購入しても同じ結果になる導線を利用者へ見せることになります。
...
**エラーからの復帰は設定の変更だけです。** 該当領域のスタンプを別のものへ差し替えれば `authorized` になります。**プレビューの再確認が必要になる**ため（`detectionRevision` は変わらないが `previewRenderHash` が変わる）、1.1 の確認からやり直します。

##### `enabledStampPacks` を認可の判定に使わない

**パックの無効化は認可では課しません。** [運用](operations.md) の 2.2 は「スタンプパックの無効化 → **新規選択だけ禁止。既存プロジェクトは再描画・再書き出しできる**」と定めています。認可で課すと、**運用側が無効化した瞬間に、そのパックを使う既存作品の再書き出しが止まります。**
...
**「変更せず再書き出し」の免除では解決できません。** 免除は `exportedSettingsEntries` の確定記録を必要とするため、**一度も書き出していないプロジェクトは対象外**です。無効化の直前に編集を終えて確認まで済ませた利用者が、書き出せなくなります。[アーキテクチャ設計](architecture.md) の 6.2 の原則「既存の作品をそのまま取り出す権利は残す」に反します。

**無効化パックが Free で使えるようになるわけではありません。** 有料パックのスタンプには `canUsePremiumStamps` が別に要ります。**`enabledStampPacks` は「どのパックを新しく選べるか」の運用レバーであり、「誰が有料スタンプを使えるか」の権限判定ではありません。** 2 つを 1 つの検査へ混ぜたのが誤りでした。

**`RemoteConfigState` の削除による迂回も、これで意味を失います。** 認可が見ないため、削除して全パックを有効へ戻しても書き出しの可否は変わりません。**新規選択の制約だけが解除され、[運用](operations.md) の 2.1 がその受容を記録しています。**

**`AppErrorCode.capabilityRequired` を追加します**（[アーキテクチャ設計](architecture.md) の 9.2）。`isRetryable` を `false` にするのは、同じ設定のまま再試行しても必ず同じ結果になるためです。**契約を変えれば通るため、Paywall からの復帰後は新しいバッチとして開始します。**

**遷移先を定めないと、その項目が `exporting` にも終端にも入らず、バッチの完了判定が成立しません。**

##### 「変更せず再書き出し」の免除

**[アーキテクチャ設計](architecture.md) の 6.2 は「既存の作品をそのまま取り出す権利は残す」と定め、有料スタンプを含むプロジェクトの「変更せず再書き出し」を降格後も可としています。** 一方 `authorizeRenderSpec` は `requirement == .premium` に対して `canUsePremiumStamps` を無条件に要求します。**どちらが優先するかを決めないと、実装者がどちらを選んでも設計の意図から外れます。**
...
**偽造できないことが前提です。** `exportedSettingsEntries` は署名済み台帳にあり、`settingsHash` は手順 −2 の認可時に計算した値を持ち回ります（上記）。**手順 3 で再計算する実装だと、停止窓で書き換えた設定のハッシュが確定記録になり、この免除がそのまま有料機能の利用権になります。** ハッシュの出所を固定することが、この免除を安全にする条件です。

**`pendingExportedSettingsEntries` を根拠にしません。** pending は手順 4 の暫定適用であり、手順 8 の昇格前はロールバックで取り消されます。**未確定の記録を認可根拠にすると、書き出しを開始して中断するだけで免除条件を作れます。**

**`canEdit` と重複しても両方を課します。** `canEdit` は編集操作の可否であり UI の判定です。**書き出しは DB の内容を直接の入力とするため、認可の側でも同じ規則を評価しなければ、DB 改変が素通りします。**

**`StampCatalog` をリモート設定から変更できません。** `enabledStampPacks` はパックの有効・無効を切り替えますが、**「どの `code` が有料か」の対応はアプリにハードコードします。** サーバー側で有料スタンプを無料へ再分類できる形にすると、[アーキテクチャ設計](architecture.md) の 10.3（リモート設定で変更できないこと）に反します。

**`ProjectSettingsHash` では代用できません。** ハッシュは「前回と同じか」しか答えず、**内容が現在の能力で許されるか**を答えません。降格前に保存したハッシュと一致していても、有料スタンプを含む設定を Free で新規に書き出すことは許されません（同 6.2 の「変更せず再書き出し」は別の判定であり、`exportedSettingsEntries` に確定記録がある場合だけ成立します）。

##### `settingsHash` は認可時の値を持ち回る

**`settingsEntryToApply.settingsHash` を手順 3 で計算し直しません。** `AccountingIntent` は手順 3 で確定しますが、その時点で DB から再計算すると、**手順 2 の `fileVerified` で停止している間に書き換えた設定が、認可も描画もされないまま台帳へ確定します。**

| 規則 | 内容 |
| --- | --- |
| 値の出所 | **手順 −2 の認可で計算した `ExportInputSnapshot.projectSettingsHash`** |
| 手順 3 での再計算 | **禁止。** `ExportInputSnapshot` から写すだけ |
| 手順 1 の再評価での検査 | **組み立てた `RenderSpec` と `ExportSetting` から計算した `ProjectSettingsHash` が、認可時の値と一致すること**（下記） |

**バックグラウンド停止に時間制限がありません。** 手順 2 の直後にアプリを背面へ送れば `fileVerified` で無期限に止まります（下記の 3 章）。その間に `EffectSetting.op` を有料スタンプへ書き換え、`Project.projectRevision` を据え置けば、`expectedProjectRevision`（手順 0 でしか照合しない）も `ExportCommit`（`RenderSpec` を持たない）も検出しません。**手順 3 で再計算すると、Free 利用者の署名済み台帳へ「有料スタンプ構成を最後に正常書き出しした」という偽の確定記録が入ります。**

**認可・描画・台帳確定の 3 者を 1 つの値で束ねます。** 手順 −2 で計算し、手順 1 で描画する `RenderSpec` に対して再検査し、手順 3 でその値をそのまま台帳へ運びます。**途中で計算し直す箇所を残すと、そこが停止窓の攻撃面になります。**
...
| 処理 | 開始条件 |
| --- | --- |
| 単体 | 現在の `projectID` / `detectionRevision` / `previewRenderHash` が `PreviewConfirmation` と**すべて一致する** |
| バッチ | 各写真について上記が一致し、`BatchReviewState.batchID` が対象バッチと一致し、かつ**モードごとの確認条件を 1.1.2 の再導出で満たす** |

**`projectID` を含めるのは、`PreviewRenderHash` が `Project` を特定しないからです。** ハッシュの入力は `RenderSpec` と `ExportSetting` だけであり（[正準スキーマ](canonical-schema.md) の 5.2）、素材 identity も `projectID` も含みません。**同じ設定・同じ領域の別プロジェクトは同じ値になります。** 含めないと、プロジェクト A で確認した値をプロジェクト B の書き出しへ渡せます。

**`overviewConfirmed` を裸の `Bool` で持ちません。** どのバッチに対する確認かを型が持たないと、別バッチの確認状態を流用できます。
...
**`WorkingSourceBinding` の照合は `transact` の外側・permit の内側で行います。** 照合はファイル I/O（ハッシュ計算）を伴い、台帳トランザクションの排他区間を数十ミリ秒延ばします。**permit の内側であれば、照合から `transact` までの間に別の書き出しが割り込むことはありません。**
...
10. 処理を開始する
11. **手順 9（コミット行の削除）**またはロールバック完了で**ゲートを解放する**
...
| 順 | 操作 | 保存先 | 遷移後の状態 |
| --- | --- | --- | --- |
| −2 | **確認の再導出（1.1.2）・設定内容の能力（1.1.1）・`WorkingSourceBinding` の照合**（[画像処理](image-pipeline.md)）を行う。いずれか不成立なら開始しない。続いて `transact` 内で時刻正規化・月次更新・期限切れ grant の整理を**永続化**する。**`SourceLease` を追加する**（勘定を問わない）。`batchTrial(true)` なら**同じトランザクション内で**追加の `TrialReservation` を作る | ファイルシステム / ProtectedBlobStore | — |
...
| 8 | 台帳トランザクションで **`pendingExportedSettingsEntries` の該当要素を `exportedSettingsEntries` へ昇格**する（[アーキテクチャ設計](architecture.md) の 6.2） | ProtectedBlobStore | — |
...
**手順 9 は手順 8 の台帳保存が成功したあとにのみ呼びます。** `ExportSagaStore` は DB ポートであり、`ProtectedBlobStore` 内の昇格完了を検査できません。**順序は `ExportCoordinator` が保証し、`deletePublished` は HMAC と `state == published` だけを再検査します。** 手順 8 の前に手順 9 を呼ぶ実装は、起動時復旧で「pending はあるが `published` コミットが無い」として手順 5.5 の削除対象になり、昇格が永久に行われません。

##### 手順 8 が繰り返し失敗する場合

**手順 8 は `ProtectedBlobStore` への書き込みであり、`temporarilyUnavailable` などで継続的に失敗しえます。** 失敗している間、コミットは `published` のまま残り、(1) 開始ゲートが解放されず（1.5）、(2) 対象 `Project` は編集禁止かつ履歴削除の絶対保護（2 章）、(3) 起動時復旧の手順 4 が完了しないため新しい書き出しも許可されません（5 章）。**アプリ全体が止まります。**

**昇格は「変更せず再書き出し」の比較対象を確定する処理であり、成果物にも会計にも影響しません。** アプリを止め続ける根拠としては弱すぎます。
...
**保護データ利用不可を「諦める」条件に含めません。** 書き出し直後に画面を消すという最も自然な操作で発火し、**利用者が無言で無料の再書き出し権を失います。** これは再試行可能なエラーとして分類されており（同 7.4）、手順 3〜7 と同じく待って再開します。

**試行回数はプロセス内カウンタで持ちます。** `ExportCommit` は署名対象なので回数を持たせると再署名が要り、**回数のために署名を作り直すのは費用が釣り合いません。** 起動をまたいでリセットされる結果、恒久的な障害では「起動ごとに 3 回試す」ことになりますが、**その 3 回はいずれも安価な台帳トランザクションであり、成功すれば昇格できます。**
...
**pending を残して手順 9 へ進みません。** 「昇格」と「削除」はどちらも 1 回の台帳トランザクションであり、**昇格が失敗する原因（`ProtectedBlobStore` への書き込み不能）は削除にも同じく効きます。** したがって削除も失敗する場合は手順 9 へ進まず、コミットを `published` のまま残します。次回起動の手順 4 が再実行します。

##### 台帳が書けない間も成果物へ到達させる

**上の帰結表（「成果物 | 影響なし」）が成立するのは、成果物への導線が実際に開く場合だけです。** 起動時復旧の手順 4 が完了しないと手順 4.2〜9 に到達せず、**手順 7.5（未受け渡し出力の復元）も手順 9（通常画面の表示）も実行されません。** 台帳への書き込み不能が続く間、公開済みの出力へ到達する手段がありません。
...
**「開始ゲート」を 2 つの意味で使いません。** ここで通すのは**起動時復旧の完了ゲート**（手順 9 に到達させ、通常画面と受け渡し導線を出すこと）です。**`ExportStartGate` の permit は 1.5 の規則どおり、手順 9（コミット行の削除）またはロールバック完了まで解放しません。**
...
**同一プロセス内では新しい書き出しを開始できません。** これは受容します。**台帳へ書けない状態では手順 −2 の `transact` も失敗するため、permit を解放しても認可は成立しません。** 守りたいのは「受け渡し前の出力へ到達できること」であり、そこに permit は要りません。**次回起動で昇格が成功すれば手順 9 が走り、permit は新しいプロセスで最初から取得されます。**

**「昇格できない」を「アプリを止める」根拠にしません。** 止めることで守られるものが無く、**利用者は受け渡し前の出力を 24 時間の期限内に取り出せなくなります。** 失うのは無料の再書き出し権だけであり、成果物より優先しません（[アーキテクチャ設計](architecture.md) の 7.5 の絶対保護と同じ向きです）。

**手順 5.5 の pending 削除保留とは独立です。** あちらは署名不正行が残る間の話であり、こちらは台帳が書けない間の話です。**どちらも「pending を消さない」ですが、前進を止めるかどうかが逆になります。**

**`deletePublished` の事前条件は「pending が解消していること」です。** 昇格でも削除でも、その `ownerExportID` の pending が台帳に無くなっていれば満たされます。**「昇格が成功したこと」ではありません。**

**利用者に不利な側へ倒す判断です。** 昇格できなかった書き出しは無料の再書き出し対象から外れます。逆に「昇格できたことにする」とアプリを止めないために未成功の設定を確定させることになり、権限の迂回になります。
...
**手順 9 を省くと、書き出しのたびにコミット行が永久に蓄積します。** ジャーナルは中断からの復旧と手順 8 の根拠のためだけに存在するので、役目を終えたら消します。
...
>
> 手順 4 で台帳へ会計を暫定適用し、**手順 7（`published` への更新）で最終確定します。**
>
...
いずれかが不成立なら手順 7 へ進まず、**`rollingBack` へ遷移してロールバックします**（4 章）。
...
**台帳側の `WorkingSourceBinding` はここでは消しません。** 手順 7 は DB 専用トランザクションです。**DB を先に消す**規則（[画像処理](image-pipeline.md)）に従い、残った binding は孤児として起動時の手順 5.5 が回収します。逆順にすると、手順 7 の直前に落ちた場合に「`WorkingSourceRecord` があり binding が無い」状態となり、正常な書き出しが差し替えとして検出されます。
...
- **手順 0 を最初に置きます。** これが無いと、手順 1 の完了後・手順 4 の前に落ちた場合に、**復旧側が「前進」と「補償」を区別できません**（下記）
- **手順 1 が失敗した場合、2 以降を実行しません。** コミットとファイルを残したまま復旧エラーとします。台帳を戻せていないのにジャーナルを消すと、消費だけが残って根拠が失われます
- **手順 1 の完了後に落ちても、再起動時に同じロールバックを冪等に再実行できます**（`rollingBack` が残っているため）
...
**ロールバックの意思を DB へ残さないと、中断で前進します。** 手順 1（台帳の取り消し）が成功し、手順 4 の前に強制終了すると、コミット行は `accountingCommitted` または `readyToPublish` のまま残り、出力ファイルは健全です。5.3 は `state` だけを見て「正常なら手順 3 からやり直す」と規定するため、**ロールバックは再実行されず、取り消したはずの会計が再適用されて公開・消費されます。** 利用者のキャンセルが無言で覆されます。
...
**遷移前の状態を保持する必要はありません。** ロールバックの操作は「台帳の `ownerExportID` が一致する要素を冪等に取り消す」であり、遷移元によって手順が変わりません（下表の各行はすべて手順 0〜5 を通ります）。

**`rollingBack` は署名対象の `state` です。** DB を書き換えて `readyToPublish` へ戻し前進させる経路を、行 HMAC が塞ぎます。

##### `markRollingBack` が事前条件を再検査する

**規則を書くだけでは強制点がありません。** `published` からのロールバックは「無い」と定めていますが（2 章）、その禁止を実行時に確かめる場所が無いと、**アプリ自身の誤りで払い戻しが走ります。**

手順 8 が失敗して指数バックオフに入っている間、コミットは `published` のまま数秒から数分残ります。**その区間に利用者がキャンセルや破棄を要求すると、`ExportCoordinator` が `markRollingBack` を呼びます。** 検査が無ければ手順 1 が `ownerExportID` 一致の消費・grant・トライアルを取り消し、手順 3 が出力ファイルを削除します。**成果物は既に写真ライブラリへ渡っているため、成果物を保ったまま枠が戻ります。**
...
**破壊的な操作はすべて、自分が前提とする状態を自分で確かめます。** 呼び出し側の順序制御に依存しません。

**UI 側でも `published` 以降はキャンセルを提示しません**（4.2）。二重の防御であり、どちらか一方に頼りません。
...
**`readyToPublish` でもロールバックが優先します。** キャンセル要求または手順 6 の検証失敗を受けた時点で `rollingBack` へ遷移し、前進しません。**「同一プロセス内なら手順 7 を実行して完了させる」という扱いは採りません。** 同じ状態でキャンセルの結果が経路によって変わると、4.2 のキャンセル境界（手順 7 より前は取り消し）と矛盾し、利用者の意思が無言で覆されます。

**手順 6 が成功して手順 7 へ進む経路と、ロールバックへ入る経路の分岐点は 1 つです。** 手順 6 の検証がすべて成立し、かつキャンセル要求が無い場合にだけ手順 7 へ進みます。
...
| 8 | **`evaluateUpdate` を実行する** | 7.5 の完了。**手順 7 の戻り値の `isUndelivered` 件数**が必要 |
...
- **`AppLifecycle` の挿入を手順 9 に置きます。** この行は手順 1 の利用痕跡判定（[アーキテクチャ設計](architecture.md) の 7.2）の入力です。**評価より後に書くことで、真の初回起動が「痕跡あり」と誤判定される経路が構造的に消えます。** 手順 9 に到達せずに終了した起動では書かれませんが、その場合は次の起動でもう一度試みられます
- **手順 3 を手順 4 より前に置きます。** 孤児予約はクレジットを占有したままなので、回収前に新しい認可を許可すると、実際には空いているクレジットを「使用中」と判定します
- **手順 5 を手順 4 の後に置きます。** 手順 4 が `OutputRecord` と `ExportRecord` を作るため（`published` への前進）、先に検査すると復旧で解消する違反を検出します
- **手順 4.5 を手順 4 の後に置きます。** コミット復旧が手順 7 を完了させると `WorkingSourceRecord` が消えるため、先に照合すると**正常に消える予定の行を差し替えとして誤検出します**
...
- **署名検証に失敗した行が 1 件でもある間は、手順 3 の孤児回収（予約・lease）と手順 5.5 の孤児 pending 削除を全件保留します**（6 章）
- **保留は「破棄して続ける」の完了後に解除し、その時点で手順 3 と 5.5 の回収を実行します**（6.1）

##### 手順 5.5 で孤児 pending を削除してよい条件

**署名不正行が存在する間、孤児 pending を削除できません。** 6.2 は「孤立 lease と孤立 pending の件数の組み合わせが到達点を一意に表す」ことを前提に自動判断します。手順 5.5 が先に pending を消すと、`(lease 1 / pending 1)` — 6.2 が「一意に決められない → 復旧エラーを維持し台帳へ触れない」と定めた状態が `(1 / 0)` に見え、**明示的に禁じた自動判断（消費確定とコミット削除）が発火します。**

| 状況 | 孤児 pending の扱い |
| --- | --- |
| 署名検証に失敗した行が 1 件以上ある | **一切削除しない**（下記） |
| 署名不正行が無い | `ownerExportID` に対応するコミット行が**存在しない** pending を削除する |
| 台帳修復が走った起動（2.5） | 台帳ごと空なので対象外 |

**保留は「孤児判定による削除」だけでなく、手順 5.5 のあらゆる pending 削除に及びます。** `Project` が存在しないことを理由とする削除も含みます。

**理由は 6.2 の判定が壊れることです。** 署名不正行の `projectID` は信用できないため、その pending が指す `Project` は `ExportCommit.projectID` の `RESTRICT`（[アーキテクチャ設計](architecture.md) の 7.1）で守られない場合があります。**その `Project` を削除すると「`Project` が無い」経路で pending が消え、6.2 の `(lease 1 / pending 1)` が `(1 / 0)` に見えます。** 保留で防いだはずの自動判断（消費確定とコミット削除）が発火します。

**`Project` が無い pending が残る間、`Project` 削除 Saga の手順 3 も pending だけを残します。** 他の 3 集合と `SourceRecord` は通常どおり削除し、pending は署名不正行が解消されるまで持ち越します。

**「有効なコミットが無い」ではなく「コミット行が存在しない」で判定します。** 復旧エラーで残った非 published の有効コミットも、署名不正で残った行も、どちらも**行としては存在する**ため、その pending は孤児になりません。`PostCommitRecoverySnapshot` は照合のために `allCommitExportIDs` を持ちます（0 章）。
...
**再選択・再接続は、署名済み `ProjectSourceSnapshot` との照合を必須とします**（[画像処理](image-pipeline.md)）。修復で `projectSourceSnapshots` が空になる以上、**この経路は必ず失敗します。** `paused` にすると復帰手段のない状態を作ります。

`failed(ledgerRepaired)` は `ExportQueueFailure(errorCode: .ledgerRepaired, isRetryable: false, occurredAt:)` の略記です（[アーキテクチャ設計](architecture.md) の 6.5 と 9.2）。**以降この略記を使います。**
...
**利用者へは「以前の作業を再開できない」ことを明示します。** `paused` のまま選び直しを促して毎回失敗させるより、理由を示して新規開始へ誘導するほうが説明できます。
...
| 対応キュー項目の特定 | **行わない。** 上の表のとおり**全非終端キュー項目**を `failed(ledgerRepaired)` にする |

**キュー項目を個別に特定しません。** 署名不正行の `projectID` は使えないため対応関係を辿れず、`exporting` の件数から推測する必要もありません。**修復起動では全非終端キュー項目が一律 `failed(ledgerRepaired)` になる**ため（上の表）、特定作業そのものが不要です。
...
**`accountingMode` を lease へ持たせるのは、これが署名済みの唯一の手掛かりだからです。** コミット行のフィールドは一切使えないため、lease に無ければ「無料枠を消費する書き出しだったのか」を判断できません。`freeMonthlyConsume` には `TrialReservation` が存在しないため、**予約の有無だけを見ると手順 4 より前にコミットを壊すことで消費を回避できます。**
...
**自動判断を拒むだけでは、復旧エラーが永久に解除されません。** 6.1 が掲げる目的（「破損したコミット 1 件でアプリが永久に書き出し不能になる」ことを避ける）が、この分岐でだけ達成されません。

**利用者が明示的に選べる最終手段を用意します。**
...
| 規則 | 内容 |
| --- | --- |
| 提示の条件 | **`(1 / 1)` または 2 件以上**の状態で、利用者が復旧エラー画面から選んだときだけ |
...
| 予約の扱い | `TrialReservation` は**削除する**（`TrialEntry` へ変換しない） |
| 署名不正行 | **同じ台帳トランザクションの完了後に、行 ID だけで削除する**（下記の 4 分岐と同じ扱い） |
| 文言 | 「中断した処理を破棄します。使用した枠は戻りません」を明示する |

**`accountingMode` から消費を確定させます。** これを省くと、**この節が塞いだはずの経路が復活します。** 上記のとおり `freeMonthlyConsume` には `TrialReservation` が存在しないため、予約の有無だけを見ると**手順 4 より前にコミットを壊すことで消費を回避できます。** 最終手段だけ確定を外すと、**コミット行を任意に削除して孤立 lease と孤立 pending の個数を作り、この分岐へ意図的に到達すれば、同じ回避が成立します。** lease 自体は正規の経路で台帳に作られた署名済みの記録であり、**対応するコミットが分からなくても「この `exportID` が無料枠を消費するつもりだった」ことは分かります。**

**払い戻しはしません。** 確定させる方向へ倒すのは、**未計上の消費を残すよりも「身に覚えのない消費が 1 件増える」ほうが害が小さいからです。** 前者は繰り返し収穫できますが、後者は 1 回きりで、かつこの分岐に到達するのは全体ゲートの不変条件が既に破れている状態です。

**トライアルクレジットは戻ります。** 予約を削除すると `trialEntries` へ移していないためクレジットが回復します。**`trialCredit` の lease に対しては `consumedExportIDs` を使わないため、ここは受容します。** 回復するのは最大 1 枚であり、正常な利用では到達しません。

**署名不正行そのものも削除します。** 削除しないと次回起動で同じ復旧エラーへ戻り、最終手段を選んだ意味がありません。**台帳の保存に成功したことを確認してから、行 ID だけで削除します**（内容を解釈しない）。

**1 件の場合に台帳の保存が失敗したら、コミットを削除せず復旧エラーを維持します。** 台帳を確定できていないのにコミットを消すと、次回起動で孤児予約として払い戻されます。

**署名不正行が存在する間は、孤児 `TrialReservation` / `SourceLease` / `pendingExportedSettingsEntries` の自動回収を全件保留します**（5 章の手順 3 と 5.5）。どの孤児が破損行に対応するかを識別できないためです。

**「破棄して続ける」の完了後に、保留していた回収を実行します。** 6.1 の最後の操作として、手順 3 と手順 5.5 の孤児回収をこの時点で走らせます。実行しないと、孤児予約がクレジットを占有したまま新規認可が許可され、**実際には空いているクレジットを「使用中」と判定します**（5 章が手順 3 を手順 4 より前へ置く理由と同じ）。
...
**`actor` であることは排他保証になりません。** `actor` は `await` のたびに再入可能であり、`MediaSaver.saveToPhotoLibrary` の待機中に別の受け渡し操作が同じ `exportID` へ入れます。

```
写真ライブラリ保存を開始（previousState = generated）
  → PhotoKit の待機中に await で中断
  → OS 共有が成功して delivered へ
  → PhotoKit が失敗
  → abandonDeliveryAttempt が previousState = generated へ戻す
  → delivered が消える
```

**`exportID` ごとの明示的な待ち行列で直列化します**（[アーキテクチャ設計](architecture.md) の 4.2 と同じ方式）。`actor` の再入性に依存しません。

| 規則 | 内容 |
| --- | --- |
| 排他の単位 | `exportID` |
| `DeliveryAttempt` が存在する間 | **その出力への共有・破棄・別の保存をすべて拒否する** |
| `abandonDeliveryAttempt` | `previousState` へ戻す。ただし**現在状態が `delivered` なら維持する** |
| `resolveOrphanedAttempts` | 同上。`delivered` を `deliveryUnknown` へ下げない |
| メソッドの分離 | 写真ライブラリ用と共有用を分ける（共有は `DeliveryAttempt` を作らない） |

**`abandonDeliveryAttempt` にも現在状態の検査を残します。** 直列化していれば理屈上は起こりませんが、**規則を 1 か所の排他実装だけに依存させません。**

##### 保存結果不明を永続化する

`previousState == .delivered` の場合、`delivered` を維持したうえで「写真ライブラリへの保存結果が不明」を提示します。**この事実は戻り値だけでは次の起動へ残りません。**
...
**追加を upsert にします。** `UnknownLibrarySave.exportID` は PRIMARY KEY です（[アーキテクチャ設計](architecture.md) の 7.1）。注記が残ったまま再び写真ライブラリ保存を試みて中断すると、次回起動の `resolveOrphanedAttempts` が同じ `exportID` を再挿入します。**この関数は単一トランザクションで全件を返す契約なので、制約違反は起動時復旧の手順 7 全体を失敗させ、7.5・8・9 へ到達できなくなります。**

**`occurredAt` を更新するのは、最後に不明になった時刻を示すためです。** 利用者への提示で「いつの保存が不明なのか」を正しく示せます。

**`OutputState` を増やしません。** 「共有は成功したが写真ライブラリは不明」は受け渡し状態の一種ではなく、`delivered` に付随する注記です。状態へ混ぜると `isUndelivered` の定義が曖昧になります。
...
**共有には `DeliveryAttempt` を作りません。** 結果が同期的に返るため中断点が無く、`.completed` のときだけ `completeShare` を呼びます。`.canceled` / `.failed` / `.unknown` では**現在の状態を変えません**（`delivered` なら `delivered` のまま）。**メソッドを分けるのは、`completeLibrarySave` が `DeliveryAttempt` の削除を伴い、`completeShare` が伴わないためです。**
