...
| 対象 | 内容 |
| --- | --- |
| 動画用データモデル | `FaceKeyframe` 等。`user-data.db` へ加算的に追加する |
| キーフレーム補間 | キーフレーム間で中心 X / 中心 Y / 幅 / 高さ / 回転角度を補間する（仕様 10.3） |
| 顔トラック関連付け / 平滑化 / シーン切替判定 | 閾値とヒューリスティクスが実際の検出結果に依存する |

**v1 で先取りしません。** 以前は「スキーマを後から足すより安い」「境界を先に切る意味がある」を理由に一部を v1 実装対象としていましたが、次の理由で外します。

- `DatabaseMigrator` による加算的なスキーマ追加は通常の運用であり、先取りの利得が小さい
- 使われないプロトコルと純粋関数が、v1 のテスト対象と実装計画を膨らませる
- 補間の入出力は動画のフレーム間隔と検出特性に依存し、実物なしに正しく作れる保証がない
