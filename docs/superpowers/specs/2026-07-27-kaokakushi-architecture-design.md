...
出力ファイルの寿命を、受け渡しの成否で分けて定義します。

| 状態 | 保持する期間 |
| --- | --- |
| 一度も受け渡しに成功していない | 利用者が明示的に破棄するまで、または 24 時間経過するまで |
| 保存または共有が 1 回以上成功した | **完了画面を離れるまで** |

したがって完了画面を開いているあいだは、保存と共有を任意の順序で何度でも実行できます。画面を離れた時点で、受け渡し済みの出力を削除します。

「あとで保存」を選んだ場合は未受け渡しのままなので、24 時間以内であれば履歴から再開できます。

保持の単位は、単体処理が直近 1 件、一括処理が直近 1 バッチ分です。

**完了画面から保存も共有もせずに離れようとした場合、確認を表示します。** これは全利用者に適用します。
...
未保存出力が残った状態でアプリが異常終了した場合、**履歴の設定と無関係に、起動時へ復旧案内を表示します。**

> 保存していない加工済み写真があります

選択肢は「保存する」「共有する」「破棄する」の 3 つです。

これは履歴の復元ではなく、**未完了の受け渡し処理の復旧**として扱います。したがって「履歴を保存しない」を選んでいる利用者にも表示します。7.2.3 が定めるとおり、未保存出力はその設定の例外だからです。

一括処理の場合はバッチ単位で案内します。

> 保存していない加工済み写真が 32 枚あります
...
**確認の成立条件を定義します。** 一覧画面を表示しただけでは、利用者が 50 枚すべてに目を通したことになりません。ただし成立条件は **6.5.5 のモードによって異なります。** 同じ確認を二度求めないためです。

**おまかせ一括**

1. 全サムネイルの生成が完了している（生成中は書き出しボタンを無効化する）
2. 一覧の末尾まで到達している
3. 要確認の写真が 1 枚も残っていない（6.5.4）
4. 利用者が「一覧の仕上がりを確認しました」を明示的に押している

**1 枚ずつ確認**

1. 全サムネイルの生成が完了している
2. **全写真について「確認済み」の操作を終えている**
3. 要確認の写真が 1 枚も残っていない
4. 完了サマリを表示する

1 枚ずつ確認では、各写真を大きく表示して個別に確認済みへ進める操作そのものが確認にあたります。そのうえで一覧の末尾到達とボタン押下まで求めるのは二重確認であり、要求しません。最終の一覧は任意の見直し用として残します。

**確認後に写真の追加削除または設定変更があった場合、確認状態を解除します。** 確認した内容と書き出す内容が一致しない状態を作らないためです。

おまかせ一括の条件 2 は、スクロール到達だけでなく VoiceOver や TalkBack による走査でも成立させます。支援技術の利用者が確認操作へ到達できない実装は、仕様 29 章に反します。

なお **末尾までの到達は安全の保証ではありません。** 見落としを減らすための手順として扱い、6.5.2 のとおり「確認済みだから安全」とは表現しません。
...
Free および Standard の利用者は、Pro の中核である一括処理を一度も試さずに月 980 円を判断することになります。これを避けるため、**一括処理でのみ使える 5 枚分のクレジット**を全プランへ付与します。
...
編集しようとした場合は、契約が必要である旨と代替を提示します。
...
- **未保存の出力ファイル**（6.2.2）。利用者がまだ受け取っていない成果物であり、履歴とは性質が異なります。保存・共有・破棄のいずれかで解消します
...
- 一覧確認の成立条件がモードごとに異なること。1 枚ずつ確認では末尾到達を求めないこと（6.5.3）
...
- 異常終了後の起動時に、履歴設定と無関係に復旧案内が出ること（6.2.2.1）
...
- 「履歴を保存しない」設定で、未保存出力と `QuotaLedger` 以外が残らないこと（7.2.3）
...
| 一括処理の体験 | Pro のみ | 全プラン共通。月間枠とは別勘定の 5 枚クレジット（6.5.6） |
