...
写真が `Reviewed` になる条件は、**その写真に付いたすべての `ReviewReason` に対して `ReviewResolution` が記録されていること**です。1 つでも未記録なら `Unreviewed` のままです。
...
「クレジットを使い切った」だけでは案内を出しません。消費済みの写真なら引き続き処理できるためです。**選択中の新しい写真の枚数が残クレジットを超えようとした時点**で出します。

`batch-credit` と `batch-size` は発火条件が近いため、**原因で分けます**。統合しないのは、提示すべき文言が異なるためです。

| 分類 | 発火条件 | 文言の趣旨 |
| --- | --- | --- |
| `batch-credit` | Free / Standard が、残クレジットを超える**新しい写真**を選ぼうとした | 「お試しで選べる新しい写真はあと N 枚です。Pro なら最大 50 枚」 |
| `batch-size` | Pro が 1 バッチ 50 枚を超えて選ぼうとした | 「1 回の一括処理では最大 50 枚まで選べます」 |

前者はアップグレードの訴求、後者は仕様上の上限の通知であり、同じ文面にはできません。
...
| 閲覧 | 可 | 可 |
| 削除 | 可 | 可 |
| 変更せず再書き出し | 可（月間枠を消費） | 可（月間枠を消費） |
| **編集して書き出し** | **可（月間枠を消費）** | Standard 以上 |
| **Free 版として複製** | — | **可** |

Free 範囲のプロジェクトとは、モザイク・ぼかし・黒塗り・基本スタンプ・手動領域・縦横比・メタデータ設定だけを使っているものです。

**「Free 版として複製」** は、有料スタンプを基本エフェクトへ置き換えた複製を作る操作です。元のプロジェクトは変更しません。これにより、有料スタンプを含む作品でも Free で編集を続ける道が残ります。

バッチに関する操作は内容によらずプランで決まります。
...
Free で再書き出しを行う場合、内容にかかわらず通常どおり月間の無料枠を消費します。
