...
たとえば 3 人が写る写真で 2 人を高信頼度・十分な大きさ・画像中央で検出し、残る 1 人を完全に見落とした場合、`triage` は空集合を返します。顔は検出されており、小さくもなく、端でもなく、重なってもいないためです。この写真は `Normal` に分類されますが、実際には 1 人の顔が露出したまま書き出されます。
...
3. `ReviewRequired` の警告がすべて解消されている
4. 完了サマリを表示する
...
**確認後に写真の追加削除または設定変更があった場合、`Reviewed` をすべて解除します。** 確認した内容と書き出す内容が一致しない状態を作らないためです。検出ステータスは検出をやり直したときにのみ再計算します。
...
利用者は各写真について、以下のいずれかの操作で `Reviewed` へ遷移させます。

- 内容を見て問題ないと判断し、確認済みにする
- 手動で隠す範囲を追加する
- 顔を隠さずそのまま保存すると明示的に選ぶ

いずれも利用者の操作を必要とします。アプリが自動的に `Reviewed` にすることはありません。検出ステータス（`ReviewRequired`）自体は変化せず、警告の理由は書き出し後も記録として残ります。
...
- 使い切るまで有効
- **1 バッチで選べる枚数は、残クレジット数を上限とする**（初期は 5 枚）
- 失敗した写真では消費しない
- 全件失敗または利用者が中止した場合、クレジットは減らない
- Pro へ加入済みの場合は消費しない

残クレジットを超える枚数を選ぼうとした時点で、Pro への案内（`batch-size`）を出します。残り 2 枚なら 3 枚目の選択で発火します。クレジットが足りないまま処理を始めさせて、途中で止まる経験を作らないためです。
...
モックは本書 v1.7 時点の仕様を反映済みです。動画の除去、確認モードの 2 分岐、検出／確認の 2 軸、出力の受け渡し状態、トライアルクレジット、メタデータの項目別設定、スタンプの参照用コピー、降格後の閲覧と変更の分離は、いずれもモック側で動作します。
