...
| 既存プロジェクトのカスタムスタンプの変更 | 不可 | 可 | 可 |
| カスタムスタンプを使った既存プロジェクトを**変更せず**再書き出し | **可** | 可 | 可 |
...
3 行目の「変更せず再書き出し」を Free でも許可する理由は、**カスタムスタンプが利用者自身の画像であり、アプリが提供したコンテンツではない**ためです。アプリ提供の追加スタンプであれば契約終了とともに利用できなくなるのが筋ですが、利用者が自分で登録した画像を使って有料期間中に完成させた作品まで取り出せなくするのは、成果物を人質に取る形になります。
