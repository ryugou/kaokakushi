...
生成済みの出力ファイルは、以下のいずれかに到達するまで一時領域に保持します。
...
**一覧確認の成立条件を定義します。** 一覧画面を表示しただけでは、利用者が 50 枚すべてに目を通したことになりません。以下をすべて満たしたときに一覧確認済みとします。
...
**一覧確認後に写真の追加削除または設定変更があった場合、確認状態を解除します。** 確認した内容と書き出す内容が一致しない状態を作らないためです。

条件 2 はスクロール到達だけでなく、VoiceOver や TalkBack による走査でも成立させます。支援技術の利用者が確認操作へ到達できない実装は、仕様 29 章に反します。
...
- 一括処理で成功した書き出し 1 枚につき 1 クレジットを消費する
...
- アプリ専用領域へ保存する
- 生成済みの出力ファイルは、保存か共有の成功、明示的な破棄、24 時間経過のいずれかまで保持する（6.2.2）
