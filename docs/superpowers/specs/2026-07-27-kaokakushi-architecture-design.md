...
一括処理の価値は「複数選択できること」ではありません。**50 枚すべてを確認せずに済むこと**です。
...
4. **問題がなさそうな写真を自動的に「確認済み」とする**
5. 利用者は「要確認」の写真だけを開いて確認する
6. 確認完了後、残りを一括書き出しする
...
`triage` が空集合を返した写真を「自動確認済み」とします。閾値はリモート設定で調整可能とします。

#### 6.5.3 仕様 10.4 との適合条件

仕様 10.4 は「書き出し前にユーザーが加工結果を確認できることを必須とする」と定めます。「確認できること」であって「毎枚の確認を必須とする」ではないため、自動確認済みは適合します。ただし成立条件が 2 つあります。

- 確認画面では、**自動確認済みの写真も加工後サムネイルとして一覧表示**し、任意にタップして開けること。要確認のみを表示して他を隠す実装は 10.4 を満たしません
- ラベルを **「自動確認済み」** とし、「安全」と表現しないこと。仕様 34.5 が完全自動を約束しないと定めている以上、トリアージを安全保証として提示できません

#### 6.5.4 一括処理モード

2 種類を定義します。

- **すべての顔を同じ方法で隠す** — 選択した全写真の全顔へ同一のエフェクトを適用する
- **写真ごとに確認してから隠す** — 6.5.1 のフローで、要確認の写真のみを確認する
...
| 機能 | 内容 |
| --- | --- |
| 要確認の抽出 | 要確認の写真だけを絞り込んで表示する |
| エラーのみ再試行 | 失敗した写真だけを再投入する |
| 処理中の追加投入 | 実行中のバッチへ写真を追加する。追加後もバッチ合計は 50 枚を超えない。超過分は次バッチへ回す |
| 一括設定プリセット | よく使う設定（エフェクト、強度、スタンプ、縦横比、背景処理、メタデータ、画質）を保存して再利用する |
| バッチ履歴 | バッチ単位で履歴に 1 件記録する（7.2.4） |
...
仕様 18.4 の 100 プロジェクト上限に加えて、保存期間の設定を提供します。
...
**保存期間の設定にかかわらず、直近 1 件のプロジェクトは 24 時間保持します。** これは仕様 14.3 の無償再書き出しの窓と一致します。この保証がないと、「履歴を保存しない」を選んだ Free 利用者が、やり直しのたびに設定を一から作り直すことになります。

期限切れによる削除の対象は、プロジェクト設定、検出結果、サムネイル、一時ファイル、キャッシュです。**写真ライブラリへ保存済みの加工済み画像は削除されません。** この点を設定画面と削除確認の両方に明示します。
...
全プラン共通で最大 100 プロジェクト。上限超過時の削除候補は「お気に入りでない」「未編集中でない」「処理キューに含まれない」「更新日時が最も古い」の全条件を満たすものとします（仕様 18.4）。7.2.3 の保存期間による削除と併用します。
...
- 履歴の保存期間判定（直近 1 件 24 時間保持の保証を含む）
- リモート設定のフォールバック
...
- **Pro の一括処理（1 バッチ 50 枚）、トリアージ、処理キュー、一括設定プリセット、バッチ履歴**
- 広告（Free のみ）
- ローカル履歴、保存期間設定、エラー復旧
...
> 旅行やイベントの写真を最大 50 枚選び、顔をまとめて検出します。確認が必要な写真だけをチェックして、一括で保存できます。
...
| 一括処理 | 素材ごとの確認が前提 | トリアージにより要確認のみ確認（6.5） |
| 履歴 | 素材のサムネイルを表示 | 加工後サムネイルのみ。保存期間設定あり。バッチは 1 件に集約（7.2） |
...
| 18.4 | 保存上限は 100 プロジェクト | 100 件上限に加えて保存期間設定（既定 30 日）。直近 1 件は 24 時間保持 | 7.2.3。プライバシー保護アプリとして未加工の顔を長期保持しない |
