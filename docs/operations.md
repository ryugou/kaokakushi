...
| `enabledStampPacks` | **バンドル同梱の全パック ID**（下記） | — | — |
| `killSwitches` | **全フラグ `false`**（何も止めない。下記） | — | — |

**`enabledStampPacks` の既定値は「同梱パックの全 ID」です。** 集合であり上限を持たないため hard max がありません。**リモート設定を取得できないときに空集合へ倒すと、有料利用者が同梱スタンプを一切使えなくなります。** 停止レバー（下記 2.2 の「スタンプパックの無効化」）は、取得に成功した設定でのみ効きます。

**`RemoteConfigState` を削除すると、無効化したパックが再び選べます。** この迂回は受容します。パックの無効化は素材の差し替えや権利上の停止に使う運用レバーであり、**`canUsePremiumStamps` が別に必要なため Free 利用者には利得がありません。** 書き出しの認可は `enabledStampPacks` を見ないため（[書き出し Saga](export-saga.md) の 1.1.1）、**解除されるのは新規選択の制約だけです。** 空集合へ倒して正当な利用者を止めるほうが害が大きいと判断します。

**`killSwitches` の既定値も全フラグ `false` です。** そして **`RemoteConfigState` を削除すると、機能停止フラグも解除されます。** これも受容します。

| 状態 | 機能停止フラグ |
| --- | --- |
| 設定を取得できている | 配信値に従う |
| **期限切れ**（`expiresAt` 超過 / `ledgerWritesSinceConfigFetch >= 4000`） | **保持する**（[アーキテクチャ設計](architecture.md) の 10.2） |
| **`RemoteConfigState` の削除** | **失われる。バンドル既定値（全 `false`）へ戻る** |

**期限切れでは守るのに削除では失われる、という非対称を受容します。** フラグを署名済み台帳（`UsageLedger`）へ写せば削除にも耐えますが、**`killSwitches` は障害対応の一時的なレバーであり、対象は一括処理・カスタムスタンプ取り込み・診断送信の 3 つに限られます**（下記 2.3）。安全性の中核に対応するキーは持ちません。**アプリ専用領域へ直接書き込める利用者が障害中の機能を動かすことは、その利用者自身のリスクに留まります。** 台帳のフィールドを増やして守る対象ではありません。

**期限切れで保持するのは、サーバー障害が「停止の解除」を意味しないためです。** これは通信できない正常な利用者に起こる状態であり、削除とは性質が違います。
