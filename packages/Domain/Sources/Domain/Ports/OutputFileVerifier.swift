...
/// `recordGeneratedOutput` へ進めない）。3検査の不成立と一時的なI/O障害の区別は
/// typed throws（`OutputFileVerificationError` 以外を throw できない）でコンパイラが強制する
/// （確定後の実体喪失検査〈export-saga.md 6章〉で、一過性の障害を「実体喪失」と誤認して
/// 完了済み出力を削除しないため）。
...
/// `OutputFileVerifier.verify` の測定結果（本計画の新設ポート）。`sha256` は
/// canonical-schema.md 5.2 のハッシュ値型3種（`ProjectSettingsHash` / `PreviewRenderHash` /
/// `StampAssetHash`）に含まれないため長さ検証を課さない（`RecordOutputInput` と同型に
/// 揃える意図的判断）。
