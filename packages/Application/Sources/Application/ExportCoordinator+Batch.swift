...
//
// startBatchItem の帰結は BatchItemStartOutcome で表す（export-saga.md 1.6 手順4の enum
...
// - itemFailed: 手順1（確認不一致）・2（能力ブロックで免除不成立）・4（権限・クォータ
//   ブロック）・5（revision 不一致）のいずれかが不成立、または手順3で `WorkingSourceRecord`
//   の行自体が存在しない。この項目のみ failed として扱われる。バッチは継続する
//
...
//   終了させバッチを止めないため（batchPaused 相当のケースは存在しない）、呼び出し元は
//   items を最後まで順番に処理すればよい。1.5「開始後の失効・昇格を無視する」は Coordinator
//   側の分岐では実現しない。バッチ内で最初に到達した項目の認可を Persistence の
//   ExportSagaStoreLive.startExport がスナップショットとして保持し、以降の同一 batchID の
//   項目はそのスナップショットをそのまま使う（一括処理キュー簡素化 Issue #40 決定2。
//   Coordinator はこの再利用を意識せず、常に authorizeAndStart を呼ぶだけでよい）。Coordinator
...
/// `startBatchItem` の帰結（export-saga.md 1.6 手順4の enum 定義と完全一致。
...
public enum BatchItemStartOutcome: Sendable {
    ...
    /// 続行する
    case itemPaused
    /// 手順1・2・4・5のいずれかが不成立（確認不一致・能力不足・権限クォータ不足・revision
    /// 不一致）、または手順3で `WorkingSourceRecord` の行自体が存在しない。該当項目だけが
    /// セッション内で failed として扱われる。バッチの残り項目は続行する
    case itemFailed
}
...
extension ExportCoordinator {
    ...
    /// 同一 batchID 内で最初に到達した項目が `ExportAuthorization` を確定させ、以降の項目は
    /// Persistence の ExportSagaStoreLive.startExport 内部でそのスナップショットをそのまま
    /// 使う（一括処理キュー簡素化 Issue #40 決定2）。Coordinator は authorizeAndStart を
